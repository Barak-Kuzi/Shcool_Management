...
const TeacherListPage = async ({searchParams}: { searchParams: Promise<SearchParams> }) => {
                ...
                <div className="flex items-center gap-2">
                    <Link href={`/list/teachers/${item.id}`}>
                        <button className="w-7 h-7 flex items-center justify-center rounded-full bg-blueSky">
                            <Image src="/view.png" alt="view button" width={16} height={16}/>
                        </button>
                    </Link>
                    {role === "admin" && (
    ...
    const [amountTeachers, teachersData] = await prisma.$transaction([
    ...
    return (
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
            ...
            {/* PAGINATION */}
            <Pagination page={currentPage} itemsQuantity={amountTeachers}/>
        </div>
    );
};
