...
type StudentWithClass = Student & { class: Class & { _count: { lessons: number; }; }; };

const SingleStudentPage = async ({params}: { params: { id: string } }) => {

    const {id} = await params;

    const student: StudentWithClass | null = await prisma.student.findUnique({
        where: {id},
        include: {
            ...
            <div className="w-full xl:w-1/3 flex flex-col gap-4">
                <div className="bg-white p-4 rounded-md">
                    ...
                    <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
                        <Link className="p-3 rounded-md bg-blueSkyLight"
                              href={`/list/lessons?classId=${student.class.id}`}>
                            Student&apos;s Lessons
                        </Link>
                        <Link className="p-3 rounded-md bg-customPurpleLight"
                        ...
                        <Link className="p-3 rounded-md bg-customYellowLight"
                              href={`/list/results?classId=${student.id}`}>
                            Student&apos;s Results
                        </Link>
                    </div>
                </div>
